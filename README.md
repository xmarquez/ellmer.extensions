...
- `chat_gemini_extended()` for Gemini file-based batches and optional
...
`batch_chat()`, `batch_chat_structured()`, and `parallel_chat()`.
...
`models_groq()` for the models available to your account and consult
Groq’s current documentation for [structured
...
`batch_chat_completed()` to check a previously submitted job, and
`ellmer::models_google_gemini()` for model discovery.

## Anthropic extended thinking with structured output

`chat_anthropic_extended()` preserves the older package interface for
combining thinking with structured output. Current ellmer versions
handle this natively; older versions use this package’s
`output_config.format` compatibility path.
