...
- `chat_groq_developer()` for Groq structured outputs and Groq batch
  support.
- `chat_gemini_extended()` for Gemini chat plus file-based batch
  support.
- `chat_anthropic_extended()` for Anthropic extended thinking with
  structured output.

The package is fully integrated with ellmer generics like
`batch_chat()`, `batch_chat_structured()`, `parallel_chat()`, etc. You
will need a paid Groq developer account for Groq batch processing, a
Gemini API key for Gemini batch processing, and an Anthropic API key for
the Anthropic extended provider.
...
Use `chat_gemini_extended()` to create a Gemini chat object with batch
support, then pass it to `batch_chat()` or `batch_chat_structured()`.
Use `batch_chat_completed()` to check whether a previously submitted
batch has finished.
...
Use `ellmer::models_google_gemini()` for a full model listing.

## Anthropic extended thinking with structured output

Standard `ellmer::chat_anthropic()` cannot combine extended thinking
(`reasoning_tokens`) with structured output because Anthropic’s API
forbids `tool_choice` with thinking enabled. `chat_anthropic_extended()`
...
`chat_anthropic_extended()` behaves identically to
